...
from .exceptions import (GrammarError, ParseError, UnexpectedCharacters,
                         UnexpectedInput, UnexpectedToken)
from .grammar import (TOKEN_DEFAULT_PRIORITY, NonTerminal, Rule, RuleOptions,
                      Symbol, Terminal)
...
from .utils import (bfs, bfs_all_unique, classify, classify_bool, dedup_list,
                    is_id_continue, is_id_start, logger, small_factors)
from .visitors import (Transformer, Transformer_InPlace,
                       Transformer_NonRecursive, Visitor, v_args)
...
class EBNF_to_BNF(Transformer_InPlace):
    ...
    def _add_recurse_rule(self, type_, expr):
        try:
            ...
            tree = ST(
                "expansions", [ST("expansion", [expr]), ST("expansion", [t, expr])]
            )
    ...
    def _generate_repeats(self, rule, mn, mx):
        ...
        if mx < REPEAT_BREAK_THRESHOLD:
            return ST(
                "expansions", [ST("expansion", [rule] * n) for n in range(mn, mx + 1)]
            )
    ...
    def expr(self, rule, op, *args):
        if op.value == "?":
            ...
            if len(args) == 1:
                ...
                if mx < mn or mn < 0:
                    raise GrammarError(
                        "Bad Range for %s (%d..%d isn't allowed)" % (rule, mn, mx)
                    )
    ...
    def expr(self, args):
        ...
        if op == "~":
            if len(args) == 3:
                ...
                if mx < mn:
                    raise GrammarError(
                        "Bad Range for %s (%d..%d isn't allowed)" % (inner, mn, mx)
                    )
                op = "{%d,%d}" % (mn, mx)
        else:
            assert len(args) == 2
        ...
        if len(set(compiled_rules)) != len(compiled_rules):
            ...
            for dups in duplicates.values():
                if len(dups) > 1:
                    ...
                    assert len({(r.alias, r.order, r.options) for r in dups}) == len(
                        dups
                    )
...
def _get_parser():
    try:
        ...
        terminals = [
            TerminalDef(name, PatternRE(value)) for name, value in TERMINALS.items()
        ]
        ...
        rules = [
            Rule(
                NonTerminal(r), [symbol_from_strcase(s) for s in x.split()], i, None, o
            )
            for r, xs, o in rules
    ...
    def do_import(
        ...
        for source in to_try:
            try:
                ...
                gb = GrammarBuilder(
                    self.global_keep_all_tokens, self.import_paths, self.used_files
                )
