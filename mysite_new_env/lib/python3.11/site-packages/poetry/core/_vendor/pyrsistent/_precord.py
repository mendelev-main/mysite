from pyrsistent._checked_types import CheckedType, InvariantException, _restore_pickle, store_invariants
...
class _PRecordMeta(type):
    def __new__(mcs, name, bases, dct):
        ...
        dct["_precord_mandatory_fields"] = set(
...
class PRecord(PMap, CheckedType, metaclass=_PRecordMeta):
    ...
    def __new__(cls, **kwargs):
        ...
        e = _PRecordEvolver(
            cls,
            pmap(pre_size=len(cls._precord_fields)),
            _factory_fields=factory_fields,
    ...
    def create(cls, kwargs, _factory_fields=None, ignore_extra=False):
        ...
        return cls(
            _factory_fields=_factory_fields, _ignore_extra=ignore_extra, **kwargs
        )
    ...
    def set(self, key, original_value):
        ...
        if field:
            if self._factory_fields is None or field in self._factory_fields:
                try:
                    if is_field_ignore_extra_complaint(
                        PRecord, field, self._ignore_extra
                    ):
                        value = field.factory(
                            original_value, ignore_extra=self._ignore_extra
                        )
                    else:
            ...
            raise AttributeError(
                "'{0}' is not among the specified fields for {1}".format(
                    key, self._destination_cls.__name__
                )
            )
