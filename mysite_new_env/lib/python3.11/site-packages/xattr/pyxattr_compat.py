...
from .lib import (XATTR_CREATE, XATTR_FINDERINFO_NAME, XATTR_MAXNAMELEN,
                  XATTR_NOFOLLOW, XATTR_NOSECURITY, XATTR_REPLACE,
                  XATTR_RESOURCEFORK_NAME, _fgetxattr, _flistxattr,
                  _fremovexattr, _fsetxattr, _getxattr, _listxattr,
                  _removexattr, _setxattr)
        ...
        try:
            if namespace is not None and namespace != _NO_NS:
                if not name.startswith(namespace):
                    continue
                result.append(
                    (name[len(namespace) :], getxattr(item, name, nofollow=nofollow))
                )
            else:
